...
trait Pipeline extends Logging {
  ...
  def outputUrl(path: String): URI = {
    ...
    parsed.getScheme match {
      case null =>
        val fullPath = s"${rootOutputUrl.getPath.reverse.dropWhile(_ == '/').reverse}/${parsed.getPath.dropWhile(_ == '/')}"
        new URI(
          rootOutputUrl.getScheme,
          rootOutputUrl.getHost,
          fullPath,
          rootOutputUrl.getFragment
        )
      case _ => parsed
    }
  }

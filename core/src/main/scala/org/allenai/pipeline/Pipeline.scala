...
trait Pipeline extends Logging {
  ...
  def persist[T, A <: Artifact: ClassTag](
    ...
    persist(original, io, createOutputArtifact[A](path))
  }
  ...
  def persist[T, A <: Artifact: ClassTag](
    ...
    persist(original, io, artifactFactory.createArtifact[A](url))
  }
  ...
  def runOne[T, A <: Artifact: ClassTag](target: PersistedProducer[T, A], outputLocationOverride: Option[String] = None) = {
    val targetWithOverriddenLocation: Producer[T] =
      outputLocationOverride match {
        case Some(tmp) =>
          target.changePersistence(target.io, artifactFactory.createArtifact[A](new URI(tmp)))
        case None => target
      }
    runOnly(
    ...
    if (config.hasPath(configKey)) {
      config.getValue(configKey).unwrapped() match {
        ...
        case path: String if path != "false" =>
          require(!persistedStepsByConfigKey.contains(stepName), "Cannot add two steps with the same name")
          val p = persist(original, io, createOutputArtifact[A](path))
          persistedStepsByConfigKey(stepName) = p
          p
        case _ => original
      }

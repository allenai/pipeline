...
class Pipeline(
    rootOutputUrl2: URI = new File(System.getProperty("user.dir")).toURI
) extends Logging {
  ...
  def persist[T, A <: Artifact: ClassTag](
    ...
    persist(original, io, outputUrl(path))
  }
  ...
  protected[this] def rootUrl = rootOutputUrl2

  def outputUrl(path: String): URI = {
  ...
  protected[this] def artifactFactory = ArtifactFactory(urlToArtifact)
  ...
  def createArtifact[A <: Artifact: ClassTag](path: String): A =
    artifactFactory.createArtifact[A](outputUrl(path))
...
trait ConfiguredPipeline extends Pipeline {
  ...
  override protected[this] def rootUrl =
    config.get[String]("output.dir").map(s => new URI(s))
    .getOrElse(super.rootUrl)
...
object ConfiguredPipeline {
  def apply(cfg: Config) = new ConfiguredPipeline {
    val config = cfg
  }

...
trait ReadHelpers extends ColumnFormats {
  ...
  def readFromArtifact[T, A <: Artifact]
  (io: ArtifactIo[T, A], artifact: A): Producer[T] = {
    require(artifact.exists, s"$artifact does not exist")
    new PersistedProducer(null, io, artifact)
  }

  /** General deserialization method. */
  def readFromArtifactProducer[T, A <: Artifact]
  (io: ArtifactIo[T, A], src: Producer[A]): Producer[T] = new Producer[T] {
    def create = io.read(src.get)
  }

  /** Read single object from flat file */
  object ReadSingleton {
    def text[T: StringSerializable]
    (artifact: FlatArtifact): Producer[T] =
      readFromArtifact(SingletonIo.text[T], artifact)

    def json[T: JsonFormat]
    (artifact: FlatArtifact): Producer[T] =
  ...
  object ReadCollection {
    def text[T: StringSerializable]
    (artifact: FlatArtifact): Producer[Iterable[T]] =
  ...
  object ReadIterator {
    def text[T: StringSerializable]
    (artifact: FlatArtifact): Producer[Iterator[T]] =
  ...
  object ReadArrayCollection {
    ...
    (artifact: FlatArtifact, sep: Char = '\t'): Producer[Iterable[Array[T]]] =
  ...
  object ReadArrayIterator {
    def text[T: StringSerializable : ClassTag]
    (artifact: FlatArtifact, sep: Char = '\t'): Producer[Iterator[Array[T]]] =
